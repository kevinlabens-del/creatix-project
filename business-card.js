...
(() => {
  ...
  function mountBusinessCard() {
    ...
    style.textContent = `
      ...
      #cr3-business-card{position:absolute;z-index:8;width:300px;padding:10px;border:1px solid rgba(127,255,185,.24);border-radius:18px;background:rgba(8,34,25,.94);box-shadow:0 18px 55px rgba(0,0,0,.38);cursor:pointer;transition:transform .2s ease,box-shadow .2s ease}
      #cr3-business-card:hover{transform:translateY(-3px);box-shadow:0 20px 65px rgba(0,0,0,.5)}
      #cr3-business-card img{display:block;width:100%;height:auto;border-radius:11px}
      ...
      #cr3-card-modal{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:24px;background:rgba(0,0,0,.88);backdrop-filter:blur(7px)}
      #cr3-card-modal.open{display:flex}
      #cr3-card-modal img{max-width:min(96vw,1500px);max-height:92vh;object-fit:contain;border-radius:12px;box-shadow:0 20px 80px #000}
      #cr3-card-close{position:fixed;right:18px;top:18px;width:46px;height:46px;border:1px solid rgba(255,255,255,.25);border-radius:50%;background:rgba(10,20,16,.82);color:white;font:28px/42px system-ui;text-align:center;cursor:pointer}
    `;
    ...
    card.innerHTML = `<img src="${CARD_IMAGE}" alt="Carte de visite CR3@TIX"><div class="label">Carte de visite</div><div class="hint">Toucher pour agrandir</div>`;
    mapWorld.appendChild(card);

    const modal = document.createElement('div');
    modal.id = 'cr3-card-modal';
    modal.innerHTML = `<img src="${CARD_IMAGE}" alt="Carte de visite CR3@TIX agrandie"><button id="cr3-card-close" aria-label="Fermer">×</button>`;
    ...
    function position() {
      ...
      const cardWidth = 300;
      const cardX = root.x + (rootWidth - cardWidth) / 2;
      const cardY = root.y + rootHeight + 180;
      card.style.left = `${cardX}px`;
      card.style.top = `${cardY}px`;

      const x1 = root.x + rootWidth / 2;
      const y1 = root.y + rootHeight;
      const x2 = cardX + cardWidth / 2;
      const y2 = cardY;
      const pad = 20;
      ...
      if (typeof WORLD === 'object' && WORLD && WORLD.height < cardY + 300) {
        WORLD.height = cardY + 360;
        mapWorld.style.height = `${WORLD.height}px`;
      }
    }
    position();
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', mountBusinessCard);
  else mountBusinessCard();
})();
