...
test('la carte pointe vers CR3@TIX SOUTIEN officiel et ne réinjecte plus AI LIVE', () => {
  const mapAdmin = read('../remote-admin.js');
  assert.match(mapAdmin, /id:\s*'soutien'/);
  assert.match(mapAdmin, /url:\s*'https:\/\/kevinlabens-del\.github\.io\/CR3-TIX-SOUTIEN-\/'/);
  assert.doesNotMatch(mapAdmin, /id:\s*'ai-live'/);
});
